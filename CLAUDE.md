...
- **Short, cohesive methods**: when a function exceeds roughly 100 lines, or its body nests past three levels, extract cohesive phases into private helpers. The *purpose* of the extraction is not the line count — it is to lower cognitive load and to open up per-phase testability. Each helper should do one thing and have a signature narrow enough to test in isolation.
- **Consistent level of abstraction within a unit**: applies at both the method level and the type level. Within a method body, prefer a single altitude, two at most — push low-level bookkeeping into named helpers so the caller reads as a sequence of phases at the same level. The same rule scales up to an implementation: when a single struct's inherent and trait impls span orchestration, domain logic, and low-level plumbing, the type is carrying too many responsibilities — decompose it into smaller, testable collaborators that the outer type delegates to. This is a guideline, not an absolute rule — dissolving every short expression into its own function, or every cohesive impl into its own struct, trades one form of cognitive load for another, since the reader must then jump between definitions to reconstruct intent. Judge by whether the level mix in a given unit hurts readability more than the extraction would. Err toward extraction when the unit is long or the levels are far apart; leave code inline when the mix is narrow enough to take in at a glance.
- **Non-trivial private helpers require tests**: when extraction produces a helper that has its own branches, panics, or state mutations, that helper must carry isolated tests covering its branches. A long function split into untested helpers is not meaningfully better than the original — the testability gain is the whole point. Purely delegating or pure-transformation helpers with no branching can remain untested if their caller's coverage is adequate.
- **No escape hatches for method length or complexity**: the codebase must remain free of `#[allow(clippy::too_many_lines)]` and `#[allow(clippy::cognitive_complexity)]` attributes. If clippy fires either lint, the correct response is to refactor — never to suppress. The annotation is a deferral tactic, not a valid resolution. Reviewers should reject any PR that introduces such an allow.
- **Every `#[allow(...)]` must carry a succinct reason**: any lint suppression that remains in the tree includes `reason = "..."` naming the invariant, bounded domain, or deliberate choice that makes the lint a non-issue at that site. One short clause is enough — state the fact, not prose around it (`"rate is validated positive; ceiling fits in u64"`, not `"this is safe because we know the rate is positive…"`). The `clippy::allow_attributes_without_reason` lint is configured at warn level and must stay clean.
