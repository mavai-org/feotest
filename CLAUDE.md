...
- **Short, cohesive methods**: when a function exceeds roughly 100 lines, or its body nests past three levels, extract cohesive phases into private helpers. Each helper should do one thing and have a signature narrow enough to test in isolation. Reach for extraction rather than `#[allow(clippy::too_many_lines)]` or `#[allow(clippy::cognitive_complexity)]` — those annotations are signals that a refactor is overdue, not a way to silence the lint.
